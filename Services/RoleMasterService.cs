...
namespace OTC.Api.Services
{
    public class RoleMasterService : IRoleMasterService
    {
        ...
        public async Task<List<RoleMaster>> GetRolesAsync()
        {
            ...
            var data = await connection.QueryAsync<RoleMaster>("India1_sp_get_roles_search", new { RoleName = (string)null, RoleStatus = (int?)null }, commandType: CommandType.StoredProcedure);

            return data.ToList();

        }

        public async Task<RoleMaster?> GetRoleByIdAsync(string name)
        {
            
            using var connection = new SqlConnection(_connectionString);


            return await connection.QueryFirstOrDefaultAsync<RoleMaster>("sp_get_roles_search",new { RoleName = name, RoleStatus = (int?)null },commandType: CommandType.StoredProcedure);
